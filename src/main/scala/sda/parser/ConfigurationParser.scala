...
object ConfigurationParser {
  ...
  def getCsvReaderConfigurationFromJson(jsonString: String): CsvReader = {
    JsonMethods.parse(FileReaderUsingI0Source.getContent(jsonString)).extract[CsvReader]
  }
  ...
  */
  def getJsonReaderConfigurationFromJson(jsonString: String): JsonReader = {
    JsonMethods.parse(FileReaderUsingI0Source.getContent(jsonString)).extract[JsonReader]
  }

  def getXmlReaderConfigurationFromJson(jsonString: String): XmlReader = {
    JsonMethods.parse(FileReaderUsingI0Source.getContent(jsonString)).extract[XmlReader]
  }
